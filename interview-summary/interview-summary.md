...
Goroutine 是一个与其他 goroutines 并行运行在同一地址空间的 Go 函数或方法。goroutine的概念类似于线程，但 goroutine是由Go的运行时（runtime）调度和管理的。Go程序会智能goroutine 中的任务合理地分配给每个CPU。它在语言层面已经内置了调度和上下文切换的机制。goroutine是Go并发设计的核心，也叫协程，它比线程更加轻量，因此可以同时运行成千上万个并发任务。在Go语言中，每一个并发的执行单元叫作一个goroutine。我们只需要在调用的函数前面添加go关键字，就能使这个函数以协程的方式运行。
...
Go 语言中，不要通过共享内存来通信，而要通过通信来实现内存共享。Go 的CSP(Communicating Sequential Process)并发模型，是通过 goroutine 和 channel 来实现的。
...
- map
  ...
  hmap 哈希表 hmap是Go map的底层实现，每个hmap内都含有多个bmap（buckets桶、oldbuckets旧桶、overflow溢出桶），既每个哈希表都由多个桶组成。
...
* 如果切片的容量小于 1024 个元素，于是扩容的时候就翻倍增加容量。一旦元素个数超过 1024 个元素，那么增长因子就变成 1.25 ，即每 次增加原来容量的四分之一。
...
Redis雪崩是指在Redis缓存层面的一种现象，当大量的数据同时过期，或者是Redis服务器宕机，导致大量的请求直接冲向数据库，可能造成数据库宕机的现象，这就是所谓的“Redis雪崩”。

解决Redis雪崩的几种策略包括：

1. **设置合理的过期策略**：避免大量的数据同时过期，可以在设置缓存的过期时间时，给每个key的过期时间增加一个随机值，这样就能避免大量的数据同时过期。
2. **使用Redis集群**：通过提供多个Redis服务，即使其中一个Redis服务挂掉，其他的服务还可以继续提供服务，从而避免全部的请求都冲向数据库。
3. **使用熔断和降级策略**：当数据库的负载达到一定的阈值时，可以通过熔断和降级的方式，暂时拒绝部分服务，或者提供部分服务，保证关键服务的正常运行。
4. **使用数据预热**：可以预先把可能会被大量访问的数据加载到缓存中，避免缓存一下子被大量的请求打空。
5. **双层缓存**：为了防止某一时刻出现大规模的缓存击穿，可以设置两层缓存，每一层缓存的失效时间不同，当第一层缓存失效的时候，请求会访问第二层缓存，这个时候可以重新生成第一层缓存。

### redis击穿

Redis击穿是指缓存中没有数据，而数据库中也没有数据，此时大量的请求直接查询数据库，从而导致数据库压力过大甚至崩溃的情况。

击穿和雪崩是不同的：

- 雪崩是指大量的缓存数据同时到达过期时间，导致大量的请求直接到数据库中去查询，可能导致数据库压力过大。
- 而击穿则是针对某一特定的key，如果这个key在数据库中不存在，而且有大量的请求不断地去请求这个key，由于缓存中也没有，所以这些请求都会去数据库中查询，可能导致数据库压力过大。

针对Redis击穿，一般有以下几种解决办法：

1. **设置短暂的缓存**：即使数据库中没有数据，也可以在缓存中设置一个短暂的缓存，表示这段时间内数据库中没有这个数据，这样可以防止大量的请求都去数据库中查询。
2. **互斥锁**：第一个查询数据库的请求可以设置一个互斥锁，让后面来的请求等待，等第一个请求查询完数据库后，释放互斥锁，让后面的请求从缓存中读取数据。
3. **使用布隆过滤器**：布隆过滤器可以有效的判断一个元素是否在一个集合中。对于所有可能会查询的key先用布隆过滤器进行一次过滤，不存在的直接返回，减少了数据库的访问压力。
...
MySQL 通过使用事务日志（transaction log）和锁机制来实现事务的 ACID 特性。事务日志记录了对数据库进行的每个操作的详细信息，以便在系统故障时进行恢复和回滚。锁机制用于保证事务之间的隔离性，防止并发执行时的数据冲突和不一致性。
...
另外，UDP也没有TCP的流量控制和拥塞控制机制，这可能会导致网络拥塞问题。
